...
function Test()
{
  let notepad_proc = Sys.Process("notepad");
  let notepad_wnd = notepad_proc.Window("Notepad", "*");
  
  let help_browser, help_wnd;
  if (aqEnvironment.GetWinMajorVersion() > 5)  
  {
    notepad_wnd.MainMenu.Click("Help|View Help");
    
    let help_proc = Sys.Process("helppane");
  ...
  else
  {
    ...
    let navigator = help_wnd.Window("HH Child", "", 2).Window("SysTabControl32");
